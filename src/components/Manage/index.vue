...
const buttons = [
  /*
  {
    ...
    title: t('manageContainer.button.addStation'),
    action: () => createModalRef.value?.open(),
  },
  */
  {
    icon: ReloadOutlined,
    title: t('manageContainer.button.reload'),
...
<template>
  <LoadingWrapper :loading="isLoading" class="w-full h-full">
    <div class="flex flex-col w-full h-full items-center body-bg rounded-[10px]">
      <div class="flex flex-col flex-1 w-full border-bottom px-4 py-4 overflow-y-auto custom-scrollbar">
        <div :key="queriedAt.toISOString()" class="flex flex-col w-full space-y-2">
          <Card
            v-for="item in stations"
            :key="item.id"
            :station="item"
            @reload="fetchAllStationInfo"
          />
        </div>
      </div>
      <!-- 按钮 -->
